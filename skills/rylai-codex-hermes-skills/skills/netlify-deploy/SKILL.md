...
- When sandboxing blocks the deployment network calls, rerun with `the runtime's approved network-access mechanism`.
...
Set `NETLIFY_AUTH_TOKEN` in the user's environment without printing or storing
the token in project files.
...
If deployment fails due to network issues (timeouts, DNS errors, connection resets), rerun the deploy with escalated permissions (use `the runtime's approved network-access mechanism`). The deploy requires escalated network access when sandbox networking blocks outbound requests.
